...
author: "Zhezhen Wang, Biniam Feleke, Qi An, Antonio Feliciano and Xinan H Yang"
...
abstract: "Heterogeneous cells within a stable state must under the control of organized gene regulation. Besides gradually and linearly changing phenomena, nonlinear critical transitions (or, tipping points) exhibit in cell-state transition (Moris 2016). This abrupt transition between two stable states has been well described in a tipping-point model (Scheffer 2012). Tipping points in complex systems may imply risks of unwanted collapse, but also opportunities for positive change. There is a state-of-the-art review with the introduction to open-source software for tipping-point prediction (http://www.early-warning-signals.org/). The purpose of this R package BioTIP is to characterize Biological Tipping-Point through analyses of a high-throughput data matrix. Generally, rows represent molecular features such as transcripts and columns represent samples in a data matrix. BioTIP implicated one new and two published methods to analyze not only longitudinal but also cross-sectional datasets. Existing R package, early-warning, was designed for longitudinal data analysis and well applicable to the ecosystem, ecology, space, and other fields. However, we found that it is hard to apply for high-throughput omics data. One published method to do biological tipping-point characterization was Dynamic Network Biomarker (DNB) (Chen 2012). This method indexes gene-oscillation per state, thus not always comparable across states to indicate a tipping point. Another published method called Index of critical transition (Ic-score) predicts tipping point based on pre-defined features (Mojtahedi 2016). Besides overcoming these limitations mentioned above, BioTIP provides functions to optimize feature pre-selection and evaluates empirical significance. Additionally, BioTIP defines transcript biotypes according the GENCODE annotation, allowing a study of noncoding transcription (Wang 2018). These improvements allow for an application to cross-sectional datasets. The BioTIP scheme acts as a hybrid model to join the advantages of both DNB and Ic-scoring, and providing enhanced features for high-throughput omics data analysis."
---
...
An existing dataset, GSE6136, is used to demonstrate how our functions are applied. Samples were a collection of transgenic mouse lymphomas divided into five groups based on clinical presentation, pathology and flow cytometry (Lenburg 2007), thus belonging to cross-sectional profiles. Noticing these five group represent a control stage similar to non-transgenic B cells and four major periods of B-cell lymphomagenesis, Dr. Chen and coauthors used the DNB method to identify the pre-disease state exists around the normal activated period (P2), i.e., the system transitions to the disease state around the transitional lymphoma period (Figure S12 in publication (Chen 2012)). 
Start by loading packages stringr, psych, and igraph. If these packages are not installed, use the `source('https //bioconductor.org/bioclite.r')`, and biocLite("packages") function to install them. Then load them using library("package"). Below are some examples.
...
```
  ...
  The summary of GSE6136_matrix is GSE6136_cli shown below.
...
<a name="Pre-selection Transcript"></a>
 ...
 __Pre-selection Transcript__
  ...
  Once normailized, we can now classify diffrernt stages. The tipping point
  which is within the "activated" state in this case. Here we see the number of
  samples that are clasified into states "activated", "lymphoma_aggressive",
...
```
  ...
  To get the selected statistics of biomodules of each state, please run the following 
...
```
  ...
  To get the selected transcript IDs, please run getBioTIP to get them.
  
```{r}
CTS = getCTS(maxMCI, maxMCIms[[2]])
```

 To visualize the trendence of every state represented by the cluster with the highest MCI scores.
```{r,echo=TRUE, warning=FALSE}
par(mar = c(10,5,0,2))
