...
#' the smallest cluster in transcripts numbers in downstream analysis. It is inherited from the parameter \code{min} of the function \code{getTopMCI}.
...
BioTIP.wrap <- function(sce, samplesL, subDir = 'newrun', 
                   smallest.population.size=20,  globa.HVG.select=FALSE,
                   ...
                   local.HVG.optimize=TRUE, localHVG.preselect.cut=0.1, localHVG.runs=100, logmat.local.HVG.testres = NULL,
                   getNetwork.cut.fdr=0.05, 
                   n.getMaxMCImember = 2,   getMCI.adjust.size =FALSE,
                   n.permutation = 100, empirical.MCI.p.cut=0.01, 
                   empirical.IC.p.cut = 0.05, local.IC.p = TRUE, IC.rank=1,
                   M=NULL,
                   ...
                   verbose=FALSE, plot=TRUE )
{
  ...
  myplotIc <- function(filename,BioTIP_scores, CTS.candidate,SimResults_g ){
    ...
    par(mfrow=c(2,n)) # 8 plots per page, SimResults_g
    for(i in 1:n){
      x = length(CTS.candidate[[i]])
      plot_Ic_Simulation(BioTIP_scores[[i]], SimResults_g[[i]], 
                         ylim=c(0, max(unlist(BioTIP_scores), na.rm=TRUE)),
  ...
  if(class(topMCI)=="numeric" & length(topMCI)>0){
    if(plot){
      ...
      # plotBar_MCI(membersL, ylim=c(0,5), minsize = 20)
      # plotBar_MCI(membersL, ylim=c(0,5), minsize = 50)
      if(!is.null(MCIbottom)) abline(h=MCIbottom, lty=2, col='red')
      dev.off()
    }
    ...
    if(length(x)>0) {
      ...
      CTS.candidate.ms <- getMaxMCImember(membersL[["members"]],
                                          membersL[["MCI"]],min =getTopMCI.gene.minsize, n=n.getMaxMCImember)
      ...
      dim(logmat)
      #[1]  3073 1362
      # M is pre-calculated gene-gene correlation matrix, will be reused in the downstream simulation analysis
      if(is.null(M)) {
