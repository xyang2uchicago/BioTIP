                                 ...
                                 control_df = NULL, control_samplesL = NULL)
{
  ...
  for(i in c(1:B)) {
    random_sample = lapply(seq_along(k), 
                     ...
                     output = c('IndexScore', 'PCCg', 'PCCs')) 
{
  ...
  if(output=='IndexScore') return(numerator / denominator)
...
#' @title Estimation of correlation matrix
#'
                      ...
                      target = c('zero',  'average', 'half')) 
{
  ...
  if (shrink) { # if we're shrinking,  find lambda and shrink towards specified target
    numerator = sum(((YtY[U] * X2tX2[U]) - (XtX[U])^2) / ((pmax(1,  YtY[U] - 1))^3))
    denominator = sum(X_cor[U]^2)
    
    lambda = ifelse(shrink,  max(0,  min(1,  numerator / denominator)),  0)
    
    if (target == 'zero') {
      target_cor = matrix(0,  nrow = nrow(X_cor),  ncol = ncol(X_cor))
    } else {
      # if (target == 'one') {
      #   target_cor = matrix(1,  nrow = nrow(X_cor),  ncol = ncol(X_cor))
      # } else {
      if(target == 'half') {
        target_cor = matrix(0.5,  nrow = nrow(X_cor),  ncol = ncol(X_cor)) 
      } else {
        avg = mean(X_cor[U])
        target_cor = matrix(avg,  nrow = nrow(X_cor),  ncol = ncol(X_cor))
      }
    }
    #}
    diag(target_cor) = 1
  ...
  if(Y.exist){
    X_cor_shrink <- X_cor_shrink[1:n1,  1:n2]
    #   cat(dim(X_cor_shrink), '\n')
  } # else X_cor_shrink <- X_cor_shrink[U] 
